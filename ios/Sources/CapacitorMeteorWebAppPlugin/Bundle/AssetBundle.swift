...
// AssetBundle.swift
//
// Manages a complete bundle of assets for a Meteor app, including
// parsing runtime configuration and handling bundle metadata.
//
...
func loadRuntimeConfigFromIndexFileAtURL(_ fileURL: URL) throws -> AssetBundle.RuntimeConfig {
  do {
    ...
    guard
      ...
      let configString = (indexFileString.substring(with: match.range(at: 1)) as NSString)
        .removingPercentEncoding,
      let configData = configString.data(using: String.Encoding.utf8)
    else {
      throw WebAppError.unsuitableAssetBundle(
        reason: "Couldn't load runtime config from index file", underlyingError: nil)
    }
    return AssetBundle.RuntimeConfig(
      json: try JSONSerialization.jsonObject(with: configData, options: []) as! JSONObject)
  } catch {
    throw WebAppError.unsuitableAssetBundle(
      reason: "Couldn't load runtime config from index file", underlyingError: error)
  }
}
...
final class AssetBundle {
  ...
  convenience init(directoryURL: URL, parentAssetBundle: AssetBundle? = nil) throws {
    ...
    try self.init(
      directoryURL: directoryURL, manifest: manifest, parentAssetBundle: parentAssetBundle)
  }
  ...
  init(directoryURL: URL, manifest: AssetManifest, parentAssetBundle: AssetBundle? = nil) throws {
    ...
    let indexFile = Asset(
      bundle: self, filePath: "index.html", urlPath: "/", fileType: "html", cacheable: false,
      hash: nil)
    addAsset(indexFile)
